...
print(f'\tVypíše všechny hodnoty z mixed_list kromě dvou posledních: {mixed_list[0:-2]}')
...
print(f'\n*************************************\nCvičení 2\n*************************************')
hunders = [number for number in range(1, 2001) if number % 200 == 0]
print(hunders)

ascii = (random.choices(string.ascii_uppercase, k=50))
print(ascii)
...
combine = tuple(zip(ascii[:len(hunders)], hunders))
print(combine)
...
persons += (('Pepik', 12, 'muž'), ('Pavel', 12, 'muž'), ('Anna', 25, 'Žena'), ('Anna2', 50, 'Žena'))
print(persons)

women2 = list(filter(lambda person: person[2] == 'Žena', persons))
...
sorted(ipeople, key=lambda person: person[2], reverse=True)
csv_string = "\n".join([f"{i + 1}. " + ";".join(map(str, row)) for i, row in enumerate(ipeople)])
